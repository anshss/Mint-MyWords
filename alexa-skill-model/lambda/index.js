    ...
    switch (attributes.permissionsError) {
      ...
      case 'permissions_required':
        return handlerInput.responseBuilder
          ...
          .withAskForPermissionsConsentCard(['alexa::profile:email:read', 'alexa::profile:name:read'])
          .getResponse();
      default:
...
const StartedInProgressMintingIntentHandler = {
  canHandle(handlerInput) {
    const { request } = handlerInput.requestEnvelope;
    return request.type === 'IntentRequest'
      && request.intent.name === 'CaptureMintText'
      && request.dialogState !== 'COMPLETED';
  ...
  async handle(handlerInput) {
    ...
    if (mintText.value) {
      // custom intent confirmation for ScheduleAppointmentIntent
      if (currentIntent.confirmationStatus === 'NONE'
        && currentIntent.slots.mintText.value) {
        const speakOutput = requestAttributes.t('APPOINTMENT_CONFIRM', mintText.value);
        const repromptOutput = requestAttributes.t('APPOINTMENT_CONFIRM_REPROMPT', mintText.value);
...
const CompletedMintingIntentHandler = {
  canHandle(handlerInput) {
    return handlerInput.requestEnvelope.request.type === 'IntentRequest'
      && handlerInput.requestEnvelope.request.intent.name === 'CaptureMintText'
      && handlerInput.requestEnvelope.request.dialogState === 'COMPLETED';
  },
  async handle(handlerInput) {
    const currentIntent = handlerInput.requestEnvelope.request.intent;
    ...
    try {
        await callDirectiveService(handlerInput, "I am sending you mail of the NFT.");
    } catch (error) {
...
async function sendEmail(name, email, prompt){
    let obj = {
        "name": name,
        "email": email,
        "prompt": prompt
    ...
    await fetch('https://alexa-skill-backend.onrender.com/api', {
        method: 'POST',
